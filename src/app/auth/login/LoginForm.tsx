        ...
        <Card className="w-full max-w-md">
          <CardContent className="p-6 space-y-5">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mt-1">请输入 2FA 验证码完成登录</p>
            </div>
            ...
            <form onSubmit={handle2FASubmit} className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button type="button" variant="ghost" className="flex-1" onClick={() => setStep("password")} disabled={loading}>
                  返回
                </Button>
                <Button type="submit" disabled={loading || otpCode.length < 6} className="flex-1 bg-primary hover:bg-primary-dark text-white">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gray-50">
      <Card className="w-full max-w-md">
        <CardContent className="p-6 space-y-5">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-gray-900">登录 TradePass</h1>
            <p className="text-sm text-gray-500 mt-1">管理您的产品与租户</p>
          </div>

          {verified && (
            <div className="p-3 bg-emerald-50 text-emerald-700 rounded-lg text-sm">邮箱验证成功，请登录</div>
          )}
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            {loginMethod === "email" ? (
              <div>
                ...
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" required className="mt-1" />
              </div>
            ) : (
              <div>
                <label className="text-sm font-medium text-gray-700">手机号</label>
                <Input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+86 138 0000 0000" required className="mt-1" />
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              ...
              <Link href="#" className="text-sm text-primary hover:underline">忘记密码？</Link>
            </div>
